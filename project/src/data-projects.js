// Project data
module.exports = {
  project1: {
    slug: 'kosmos',
    title: 'KMPG',
    desc: 'UI reskin of internal project for banking group, including CSS architecture updates and Animations/Transitions',
    client: 'KPMG',
    agency: 'Fathom London',
    tech: 'Angular (4) + Typescript JavaScript, CSS',
    tools: 'Angular CLI, SASS, Yarn, Sketch, NPM, Travis, Github, Docker',
    date: '2017'
  },

  project2: {
    slug: 'boden',
    title: 'Boden Styleguide',
    desc: 'Creation of a Frontend PatternLab component library (Gulp+Mustache flavour) for a retail redesign + documentation.',
    client: 'Boden',
    agency: 'BBH London',
    tech: 'JavaScript ES6, CSS, CSS & SVG animations',
    tools: 'NPM, PatternLab (Gulp), PostCSS, PHP, CircleCI, Github, Sketch',
    date: '2017',
    gallery: '',
    site: ''
  },

  projectlipton: {
    slug: 'lipton',
    title: 'Lipton',
    image: 'images/lipton.png',
    desc: 'Styling various UI elements for Tea brand site.',
    client: 'Unilever',
    agency: 'Tribal Worldwhite',
    tech: 'CSS, JavaScript',
    tools: 'Sass, RequireJS, AEM, Whitelabel Unilever platform',
    date: '2017',
    gallery: '',
    site: 'www.lipton.com/'
  },

  project3: {
    slug: 'ilk',
    title: 'ilk London',
    image: 'images/ilk.png',
    desc: 'Established a Frontend theme and component library for a Magento ecommerce web site',
    client: 'ilk',
    agency: 'BBH London',
    tech: 'Magento (PHP/XML), CSS, JavaScript ES6',
    tools: 'Magento, NPM, PostCSS, RequireJS, Vagrant',
    date: '2016',
    gallery: '',
    site: 'www.ilklondon.com/'
  },

  project4: {
    slug: 'hunt-the-hacker',
    title: 'Hunt the Hacker',
    image: 'images/hacker.png',
    desc: 'Built and styled small game-like apps inside an OS.js project.',
    client: 'Mattesons',
    agency: 'Stink Studios',
    tech: 'JavaScript ES6, CSS',
    tools: 'OS.js, LESS, NPM',
    date: '2016',
    gallery: '',
    site: 'mattessons.co.uk/hunt-the-hacker'
  },

  project5: {
    slug: 'paypal',
    title: 'PayPal',
    image: 'images/paypal.png',
    desc: 'Various templated marketing microsite UI builds for leading online payments institution.',
    client: 'PayPal',
    agency: 'Lewis Purestone',
    tech: 'JavaScript, CSS',
    tools: 'Underscore, Nunjucks templates, jQuery, NPM, PostCSS',
    date: '2016',
    gallery: '',
    site: ''
  },

  project6: {
    slug: 'fom',
    title: 'Force Over Mass',
    image: 'images/fom02.png',
    desc: 'Contributed to Front end build and styling of CraftPHP project',
    client: 'Force Over Mass Capital',
    agency: 'Ideas Made Digital Ltd',
    tech: 'JavaScript, CSS, CSS & SVG animations',
    tools: 'CraftPHP, Git, PostCSS, NPM, Photoshop, Vagrant',
    date: '2015',
    gallery: '',
    site: 'fomcap.com/'
  },

  project7: {
    slug: 'guardian',
    title: 'Christmas Gift Guide 2015',
    image: 'images/guardian.png',
    desc: 'Built and styled Backbone microsite allowing readership to browse and favourite curated Christmas Gift shop.',
    client: 'the guardian',
    agency: 'Ideas Made Digital',
    tech: 'JavaScript ES6, CSS, CSS & SVG animations',
    tools: 'Backbone, Handlebars, Git, PostCSS, NPM, Photoshop, SASS, Google Docs (Extra light CMS)',
    date: '2015',
    gallery: '',
    site: 'theguardian.com/lifeandstyle/ng-interactive/2016/dec/01/christmas-gift-guide-2016-shopping-buy-online'
  },

  project8: {
    slug: 'abbey-road',
    title: 'Inside Abbey Road',
    image: 'images/google.jpg',
    desc: 'Contributed to the build and styling of a Backbone component library, including animations',
    client: 'Google',
    agency: 'Stink Studio',
    tech: 'JavaScript, CSS',
    tools: 'Backbone.js, Marionette, NPM, Stylus, PostCSS',
    date: '2015',
    gallery: '',
    site: 'insideabbeyroad.withgoogle.com/en'
  },

  project9: {
    slug: 'showstudio',
    title: 'SHOWstudio',
    image: 'images/showstudio.jpg',
    desc: 'Frontend developer for fashion photographer Nick Knight\'s online platform for fashion film, art + ecommerce. Built a range of projects, from the first live-streamed fashion show (Alexander McQueen, 2009) to online live interviews with Lady Gaga, Gareth Pugh, and others.',
    agency: 'SHOWstudio',
    client: 'Various',
    tech: 'JavaScript, CSS, HTML5, PHP',
    tools: 'Nginx, CakePHP, jQuery, Git, Vagrant, Photoshop',
    date: '2009-2014',
    gallery: '',
    site: 'showstudio.com'
  }
};
